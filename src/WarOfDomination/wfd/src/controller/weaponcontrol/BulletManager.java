...
public class BulletManager {
	...
	/*
	 * This function creates a bullet object to track the projectile. It uses a
	...
	/*
	 * This function is the same as shoot() function of this class. However this one
	 * creates a "Shotgun" weapon effect by adding 5 bullets at once to the
	 * listOfBullets. Shotgun effect is created by randomizing the angle slightly.
	 */
	public void shootShotgun(int x, int y, float sourceX, float sourceY, Moving shooter, Moving enemy)
	...
	/*
	 * This function deletes the bullet if it collides with a player or an obstacle
	 * on the map. This function updates a SINGLE bullet at a time.
	 */
	public void updateBullet(Weapon aBullet, float mapPosX, float mapPosY, TiledMap map) {
		...
		// Checks if the bullets are outside of the map or not. Deletes bullets if
		// outside.
		if (((aBullet.getX() - mapPosX) / 32 > 0) && ((aBullet.getY() - mapPosY) / 32 > 0)
				&& ((aBullet.getY() - mapPosY) / 32 < map.getHeight())
				&& ((aBullet.getX() - mapPosX) / 32 < map.getWidth())) {
			...
			if (map.getTileId((((int) (aBullet.getX() - mapPosX) / 32)), ((int) (aBullet.getY() - mapPosY) / 32),
				...
				if (Math.hypot(characterX - aBullet.getX(), characterY - aBullet.getY()) < 18) {
