...
public class MapControl {
	 ...
	 * Basically this function disables a character from moving closer than half of it's size to any obstacle on any map in our game.
	 * Function returns false if a character is too close to an obstacle in the specified direction.
	 */
	public boolean canMove(Moving ch, float delta, Direction dir) {
		

		//Only should be initialized once; -10000 is a layer number that doesn't exists in our map. 
		//Since both characters calls this method we don't want to re-initialize layers for each of them.
		if(layerIndex == -10000)
		{
		...
		if (dir == Direction.UP) {
			if((ch.getY() - this.mapY + 8 + delta * .1f)  > 8)//Outside map check
				...
				if (map.getTileId(((int) (ch.getX() - this.mapX) / 32), (((int) (ch.getY() - this.mapY - 8 - delta * .1f) /32)),
						layerIndex) != object) {
					return true;
			}
			...
			if((ch.getY() - this.mapY + 8 + delta * .1f) / 32  < this.map.getHeight())//Outside map check
				//Obstacle collision check
				if (map.getTileId(((int) (ch.getX() - this.mapX) / 32),((int) (ch.getY() - this.mapY + 8 + delta * .1f) / 32), layerIndex) != object)
					return true;
