...
public class PickupManager {
	...
	// Checks collision between characters and pickup objects. Objects are "picked
	// up" when collided.
	public void update(Moving character1, Moving character2) {
		...
		for (int i = 0; i < listOfAmmoBoxes.size(); i++) {
			if (Math.hypot(character1X - listOfAmmoBoxes.get(i).getX(),
					character1Y - listOfAmmoBoxes.get(i).getY()) < GRAB_RADIUS) {
				// reload character
				character1.reload();
				listOfAmmoBoxes.remove(i);
			}

			else if (Math.hypot(character2X - listOfAmmoBoxes.get(i).getX(),
					character2Y - listOfAmmoBoxes.get(i).getY()) < GRAB_RADIUS) {
				// reload character
				character2.reload();
				listOfAmmoBoxes.remove(i);
			}
		...
		for (int i = 0; i < listOfHealthBoxes.size(); i++) {
			if (Math.hypot(character1X - listOfHealthBoxes.get(i).getX(),
					character1Y - listOfHealthBoxes.get(i).getY()) < GRAB_RADIUS) {
				// heal character
				character1.heal(listOfHealthBoxes.get(i).getAmount());
				listOfHealthBoxes.remove(i);
			}

			else if (Math.hypot(character2X - listOfHealthBoxes.get(i).getX(),
					character2Y - listOfHealthBoxes.get(i).getY()) < GRAB_RADIUS) {
				// heal character
