...
public class GameManager {
	...
	{
		Vector2f diffvector = new Vector2f((float)( aBullet.targetgetX()-aBullet.getX()),(float) (aBullet.targetgetY()-aBullet.getY()));
		float angle = (float) diffvector.getTheta();
		
		if(((aBullet.getX() - screenPosX)/32 > 0) && ((aBullet.getY() - screenPosY)/32 > 0) && ((aBullet.getY() - screenPosY)/32 < 20) && ((aBullet.getX() - screenPosX)/32 < 20))
		{
			if(map.getTileId((((int) (aBullet.getX() - screenPosX)/32)) , ((int) (aBullet.getY() - screenPosY)/32)  , objectLayer) != 65)
			{
				if(Math.hypot(aBullet.targetgetX()-aBullet.getX(), aBullet.targetgetY()-aBullet.getY()) > 3) 
				{
